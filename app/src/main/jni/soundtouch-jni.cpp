...
extern "C" DLL_PUBLIC jstring Java_me_lake_soundtouch_SoundTouch_getVersionString(JNIEnv *env,
                                                                                  jobject thiz) {
    const char *verStr;
...
extern "C" DLL_PUBLIC jlong Java_me_lake_soundtouch_SoundTouch_newInstance(JNIEnv *env,
                                                                           jobject thiz) {
    return (jlong)(new SoundTouch());
}


extern "C" DLL_PUBLIC void Java_me_lake_soundtouch_SoundTouch_deleteInstance(JNIEnv *env,
                                                                             jobject thiz,
                                                                             jlong handle) {
...
extern "C" DLL_PUBLIC void Java_me_lake_soundtouch_SoundTouch_setTempo(JNIEnv *env, jobject thiz,
...
extern "C" DLL_PUBLIC void Java_me_lake_soundtouch_SoundTouch_setPitchSemiTones(JNIEnv *env,
                                                                                jobject thiz,
                                                                                jlong handle,
                                                                                jfloat pitch) {
    SoundTouch *ptr = (SoundTouch *) handle;
...
extern "C" DLL_PUBLIC void Java_me_lake_soundtouch_SoundTouch_setSpeed(JNIEnv *env, jobject thiz,
...
extern "C" DLL_PUBLIC jstring Java_me_lake_soundtouch_SoundTouch_getErrorString(JNIEnv *env,
                                                                                jobject thiz) {
...
extern "C" DLL_PUBLIC int Java_me_lake_soundtouch_SoundTouch_processFile(JNIEnv *env, jobject thiz,
                                                                         jlong handle,
                                                                         jstring jinputFile,
                                                                         jstring joutputFile) {
    SoundTouch *ptr = (SoundTouch *) handle;
...
unsigned char *pShort2pByte(short *p) {
    return (unsigned char *) p;
}

short *pByte2pShort(unsigned char *p) {
    return (short *) p;
}
extern "C" DLL_PUBLIC int Java_me_lake_soundtouch_SoundTouch_setInputConfig(JNIEnv *env, jobject thiz,
...
extern "C" DLL_PUBLIC int Java_me_lake_soundtouch_SoundTouch_putData(JNIEnv *env, jobject thiz,
                                                                     jlong handle, jbyteArray data,
                                                                     jint samples) {
    SoundTouch *pSoundTouch = (SoundTouch *) handle;
    unsigned char *audioData = (unsigned char *) env->GetByteArrayElements( data, 0);
    LOGV("JNI exception 3,%d",sizeof(SAMPLETYPE));
    if (_init_threading(true)) return -1;
    try {
        pSoundTouch->putSamples((SAMPLETYPE*)pByte2pShort(audioData), samples);
    } catch (const runtime_error &e) {
        const char *err = e.what();
        LOGV("JNI exception in SoundTouch::processFile: %s", err);
        _setErrmsg(err);
...
extern "C" DLL_PUBLIC int Java_me_lake_soundtouch_SoundTouch_getData(JNIEnv *env, jobject thiz,
                                                                     jlong handle, jbyteArray data,
                                                                     jint samples) {
    SoundTouch *pSoundTouch = (SoundTouch *) handle;
    unsigned char *audioData = (unsigned char *) env->GetByteArrayElements( data, 0);
    LOGV("audio,%d,%d,%d,%d",audioData[0],audioData[1],audioData[2],audioData[3]);
    SAMPLETYPE* s = (SAMPLETYPE*)pByte2pShort(audioData);
    LOGV("audio2,%d,%d,%d,%d",s[0],s[1],s[2],s[3]);

    if (_init_threading(true)) return -1;

    int nSamples=-1;
    try {
        nSamples = pSoundTouch->receiveSamples((SAMPLETYPE*)pByte2pShort(audioData), samples);
    } catch (const runtime_error &e) {
        const char *err = e.what();
        LOGV("JNI exception in SoundTouch::processFile: %s", err);
        _setErrmsg(err);
